...
// ErrInvalidArtifactTTL is returned when LOSTFILM_ARTIFACT_TTL is not a valid
// positive Go duration.
var ErrInvalidArtifactTTL = errors.New("LOSTFILM_ARTIFACT_TTL must be a positive Go duration (e.g. 15m)")
...
type Config struct {
	...
	// URLs (e.g. http://mcp-lostfilm.internal:9090). Empty derives it from the
...
func Load() (*Config, error) {
	...
	artifactTTL, ttlErr := parseArtifactTTL(os.Getenv("LOSTFILM_ARTIFACT_TTL"))
	if ttlErr != nil {
		return nil, ttlErr
	}
	...
	return &Config{
		...
		Proxy:           proxy,
		ArtifactBaseURL: os.Getenv("LOSTFILM_ARTIFACT_BASE_URL"),
...
// parseArtifactTTL parses LOSTFILM_ARTIFACT_TTL, defaulting to
