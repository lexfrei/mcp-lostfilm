...
const sampleTorrent = "Episode.torrent"
...
func TestArtifactHandler_ServesOnce(t *testing.T) {
	...
	art, err := store.Put(sampleTorrent, []byte("dabc"))
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
	...
	wantCD := `attachment; filename="` + sampleTorrent + `"; filename*=UTF-8''` + sampleTorrent
...
func TestRFC5987Value_PreservesCyrillic(t *testing.T) {
	...
	if got := rfc5987Value(sampleTorrent); got != sampleTorrent {
...
func TestNewServerOptions_HasInstructions(t *testing.T) {
	...
	// The instructions must describe the current download delivery via 'mode',
	// which the agent reads to decide how to call the tool.
